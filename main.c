...
void drawAPrism(RectPrism rectPrism){
    ...
    /* Draw top side */
    topStartingYPoint = rectPrism.yloc - rectPrism.length - 15;
    bottomStartingYPoint = rectPrism.yloc - rectPrism.length - 2;
    xOffset = 0;
    ...
    int topToShadeY = rectPrism.yloc - rectPrism.length - 1;
    ...
    topToShadeY = rectPrism.yloc - rectPrism.length - 2;
    for(i = topToShadeX; i >= topToShadeX - 6; i--, topToShadeY--){
    ...
    int topToBrightY = rectPrism.yloc - rectPrism.length - 1;
    ...
    topToBrightY = rectPrism.yloc - rectPrism.length - 2;
    for(i = topToBrightX; i <= topToBrightX + 6; i++, topToBrightY--){
    ...
    for(i = 0; i < 6; i++){
        ...
        for(j = yMiddle - rectPrism.length; j <= yMiddle + rectPrism.length; j++){
    ...
    for(i = rectPrism.yloc - rectPrism.length; i <= rectPrism.yloc + rectPrism.length; i++){
        plot_pixel(rectPrism.xloc - 1, i, SHADE_TO_LIME_1);
    }

    /* Draw a pixel between middle part of side and top */
    plot_pixel(rectPrism.xloc, rectPrism.yloc - rectPrism.length - 1, MIDDLE_TO_TOP);



    /* Draw boundary between shaded and lime side - 2 */
    for(i = rectPrism.yloc - rectPrism.length; i <= rectPrism.yloc + rectPrism.length + 1; i++){
